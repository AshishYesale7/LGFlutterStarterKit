...
> **📋 Recommended Plugins for Task 2**: The `demo/DEPENDENCIES.md` file in this starter kit contains the verified plugin stack from a working LG Task 2 demo, including exact versions, platform compatibility matrix, and a ready-to-use `pubspec.yaml` template. Key plugins: `dartssh2` (SSH to rig), `google_maps_flutter` (in-app map), `xml` (KML generation), `provider` (state management), `shared_preferences` + `flutter_secure_storage` (settings & passwords), `path_provider` (KML file I/O). See `demo/flutter-plugins.txt` and `demo/flutter-plugins-dependencies.txt` for the full resolved dependency graph.
